...
<div class="container-fluid px-4">
    <h1 class="mt-4">Archived Request </h1>
            ...
            <div class="card-body" style="overflow-x:auto">
                <table id="view-assigned-bookings" class="table table-bordered">
                    ...
                    <tbody>
                        <?php
                        $declined = 'declined';
                        $query="SELECT bookings.booking_id AS ID, users.Name AS Name, bookings.service AS Service, bookings.reserve_date AS Requested_Date, declined_bookings.reason_for_decline AS Reason, declined_bookings.timestamp AS Timestamp FROM bookings JOIN users ON bookings.user_id = users.id LEFT JOIN declined_bookings ON bookings.booking_id = declined_bookings.booking_id WHERE bookings.status = ?";
                        if ($stmt = mysqli_prepare($con, $query)) {
                            mysqli_stmt_bind_param($stmt, "s", $declined);
